...
# Create Compute Resource Group
 resource "azurerm_resource_group" "ComputeRG" {
   name     = "Compute-RG-${random_integer.ri.result}"
   location = "East US"
   tags = {
     environment = "staging"
     Builder = "obalogun"
...
# Create Frontend Network Security goup
 resource "azurerm_network_security_group" "FENSG" {
  name = "FE-NSG-${random_integer.ri.result}"
  location            = azurerm_resource_group.vnRG.location
   resource_group_name = azurerm_resource_group.vnRG.name
  tags = {
     environment = "staging"
...
# Create Mid level Network Security goup
 resource "azurerm_network_security_group" "MIDNSG" {
  name = "MID-NSG-${random_integer.ri.result}"
  location            = azurerm_resource_group.vnRG.location
   resource_group_name = azurerm_resource_group.vnRG.name
  tags = {
     environment = "staging"
...
# Create Backend Network Security goup
 resource "azurerm_network_security_group" "BENSG" {
  name = "BE-NSG-${random_integer.ri.result}"
  location            = azurerm_resource_group.vnRG.location
   resource_group_name = azurerm_resource_group.vnRG.name
  tags = {
     environment = "staging"
...
# Create Network security groups.  Add rules to each resource as needed
 ...
 resource "azurerm_subnet_network_security_group_association" "MIDNSGAssign" {
  ...
  network_security_group_id = azurerm_network_security_group.MIDNSG.id
}

 resource "azurerm_subnet_network_security_group_association" "BENSGAssign" {
  subnet_id                 = azurerm_subnet.BESubnet.id
  network_security_group_id = azurerm_network_security_group.BENSG.id
}

# Create storage account for logging
...
# Create Data Disk..  comment the next batch to remove this 
...
resource "azurerm_windows_virtual_machine" "VM" {
   ...
   os_disk {
    name                 = "myOsDisk${count.index}${random_integer.ri.result}"
    caching              = "ReadWrite"
    storage_account_type = "Premium_LRS"
  }

  # configure boot diagnostics for VM
  boot_diagnostics {
...
# Deploy Monitoring Resource Group
...
#NEED TO INSERT DATA GET SCRIPT TO PASS NETWORK INTERFACE ip ADDRESSES

# associate to Backend Pool
resource "azurerm_network_interface_backend_address_pool_association" "addBEpool" {
  network_interface_id    = azurerm_network_interface.nic[count.index].private_ip_address
